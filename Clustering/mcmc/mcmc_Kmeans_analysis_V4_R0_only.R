# MCMC K-Means Clustering Analysis V4 - R0 Only Sensitivity Analysis
...
# --- 3. Load Data ---
params_df <- read_csv("Clustering/mcmc/Kmeans/pathogen_params_V2_extended.csv")
...
for (iter in 1:N_MCMC_ITERATIONS) {
  ...
  for (i in 1:nrow(params_df)) {
    ...
    k_sampled <- sample_clust_parameter(pathogen_row, "k")
    lp_sampled <- sample_clust_parameter(pathogen_row, "LatentPeriod")
    ip_sampled <- sample_clust_parameter(pathogen_row, "InfectiousPeriod")
    ...
    pathogen_params_df <- data.frame(
      ...
      LatentPeriod_sampled = ifelse(is.null(lp_sampled) || is.na(lp_sampled) || !is.finite(lp_sampled), NA_real_, lp_sampled),
      InfectiousPeriod_sampled = ifelse(is.null(ip_sampled) || is.na(ip_sampled) || !is.finite(ip_sampled), NA_real_, ip_sampled),
...
if (length(mcmc_results) > 0 && !all(sapply(mcmc_results, is.null))){
    ...
    if(nrow(all_mcmc_samples_df) > 0){
        ...
        # --- 7. Save Results ---
        write_csv(all_mcmc_samples_df, "Clustering/mcmc/Kmeans/mcmc_parameter_samples_v4_R0_only.csv")
        print("MCMC parameter samples saved to Clustering/mcmc/Kmeans/mcmc_parameter_samples_v4_R0_only.csv")
    } else {
...
if (exists("all_mcmc_samples_df") && nrow(all_mcmc_samples_df) > 0) {
  ...
  clustering_data_full <- all_mcmc_samples_df %>%
    select(
      ...
      Pathogen_Name,
      R0_sampled 
      # SI_Clust_sampled, # V4 - R0 Only
      # CFR_sampled, 
      # # Presymp_Proportion_sampled, # Excluding for V3 sensitivity analysis
      # k_sampled,
  ...
  numerical_param_cols <- c("R0_sampled") # V4 - R0 Only
      ...
      if (!is.null(kmeans_iter_result)) {
          ...
          iter_unscaled_centroids <- original_data_this_iter_for_centroids %>%
            ...
            summarise(
              ...
              # SI_Clust_sampled = mean(SI_Clust_sampled, na.rm = TRUE), # V4 - R0 Only
              # CFR_sampled = mean(CFR_sampled, na.rm = TRUE),
              # # Presymp_Proportion_sampled = mean(Presymp_Proportion_sampled, na.rm = TRUE), # Excluding for V3
  ...
  if (length(all_iteration_centroids) > 0 && length(all_iteration_assignments) > 0) {
      ...
      if(length(feature_cols_for_centroids) > 0 && nrow(combined_centroids_df) > 0) {
          ...
          write_csv(centroid_summary, "Clustering/mcmc/Kmeans/cluster_centroids_summary_with_ci_k6_v4_R0_only.csv")
          print("Cluster centroids summary saved to Clustering/mcmc/Kmeans/cluster_centroids_summary_with_ci_k6_v4_R0_only.csv")
      } else {
      ...
      if (nrow(combined_assignments_df) > 0) {
          ...
          write_csv(modal_assignments, "Clustering/mcmc/Kmeans/pathogen_modal_cluster_assignments_k6_v4_R0_only.csv")
      } else {
      ...
      pathogen_summary_for_plot <- all_mcmc_samples_df %>%
          ...
          summarise(
              ...
              # Presymp_Proportion_mean_overall = mean(Presymp_Proportion_sampled, na.rm = TRUE), # Excluding for V3
      ...
      # --- NOTE FOR V4 SENSITIVITY ANALYSIS ---
      # A PCA plot is not meaningful for a single clustering variable (R0). 
      # The first principal component would simply be the scaled R0 values.
      # Therefore, this visualization is skipped for the R0-only analysis.
      #
      # plot_features_numerical <- c("R0_mean_overall") # V4 - R0 only
      # features_for_pca_plot <- pathogen_summary_for_plot %>%
      #     select(all_of(plot_features_numerical))
      # ... (rest of PCA code is commented out) ...
      ...
      if (length(mcmc_iterations) > 0) {
          if (!requireNamespace("cluster", quietly = TRUE)) {
              ...
              print("Package 'factoextra' is not installed, cannot generate silhouette plot.")
          } else {
            library(cluster)
            library(factoextra)
            ...
            if (nrow(example_iter_features) >= CHOSEN_K) {
                ...
                if(!is.null(kmeans_example_iter_result)){
                        ...
                        ggsave(paste0("Clustering/mcmc/Kmeans/silhouette_plot_k",CHOSEN_K,"_iter",example_iter_val,"_v4_R0_only.png"), plot = sil_plot_example_iter, width=8, height=6)
...
if (exists("combined_assignments_df") && nrow(combined_assignments_df) > 0 && 
  ...
  write.csv(coassignment_matrix, "Clustering/mcmc/Kmeans/coassignment_matrix_v4_R0_only.csv")
  ...
  write.csv(dissimilarity_matrix, "Clustering/mcmc/Kmeans/dissimilarity_matrix_v4_R0_only.csv")
  ...
  png_filename <- paste0("Clustering/mcmc/Kmeans/figure.1_v4_R0_only", ".png")
  ...
  write.csv(consensus_assignments_df, paste0("Clustering/mcmc/Kmeans/consensus_cluster_assignments_k", K_CONSENSUS, "_v4_R0_only.csv"))
  ...
  if(nrow(pathogen_mcmc_samples_with_consensus_clusters) > 0 && "Consensus_Cluster" %in% names(pathogen_mcmc_samples_with_consensus_clusters)){
    ...
    if(length(consensus_cluster_summary_list) > 0){
      ...
      write_csv(final_consensus_summary_df, paste0("Clustering/mcmc/Kmeans/consensus_clusters_summary_k", K_CONSENSUS, "_v4_R0_only.csv"))
      print(paste0("Consensus clusters summary saved to Clustering/mcmc/Kmeans/consensus_clusters_summary_k", K_CONSENSUS, "_v4_R0_only.csv"))
    } else {
