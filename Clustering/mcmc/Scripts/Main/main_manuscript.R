...
if (nrow(all_mcmc_samples_df) > 0) {
    ...
    print("MCMC parameter samples saved to Clustering/mcmc/Kmeans/main_outputs/mcmc_parameter_samples.csv")
} else {
  ...
  if (length(successful_results) > 0) {
    ...
    if (nrow(all_iteration_assignments) > 0) {
      ...
      if (!requireNamespace("ggplot2", quietly = TRUE)) {
          ...
          ggsave(paste0("Clustering/mcmc/Kmeans/main_outputs/pca_modal_cluster_plot_k",CHOSEN_K,".png"), plot = pca_plot_modal, width=10, height=8)
          print(paste0("PCA plot with modal cluster assignments saved to Clustering/mcmc/Kmeans/main_outputs/pca_modal_cluster_plot_k",CHOSEN_K,".png"))
            ...
            if (nrow(example_iter_features) >= CHOSEN_K) {
                ...
                if(!is.null(kmeans_example_iter_result)){
                    if (!requireNamespace("cluster", quietly = TRUE)) {
                        ...
                        ggsave(paste0("Clustering/mcmc/Kmeans/main_outputs/silhouette_plot_k",CHOSEN_K,"_iter",example_iter_val,".png"), plot = sil_plot_example_iter, width=8, height=6)
...
if (exists("all_iteration_assignments") && nrow(all_iteration_assignments) > 0 && 
  ...
  coassignment_heatmap_filename <- "Clustering/mcmc/Kmeans/main_outputs/coassignment_heatmap_S7.png"
  ...
  results_filename_optimal_k <- "Clustering/mcmc/Kmeans/main_outputs/optimal_k_silhouette_results.csv"
  ...
  dendro_figure_filename <- "Clustering/mcmc/Kmeans/figures/figure.3.png"
  ...
  write.csv(consensus_assignments_df, paste0("Clustering/mcmc/Kmeans/main_outputs/consensus_cluster_assignments_k", K_CONSENSUS, ".csv"))
  ...
  if(nrow(pathogen_mcmc_samples_with_consensus_clusters) > 0 && "Consensus_Cluster" %in% names(pathogen_mcmc_samples_with_consensus_clusters)){
    ...
    if(length(consensus_cluster_summary_list) > 0){
      ...
      write_csv(final_consensus_summary_df, paste0("Clustering/mcmc/Kmeans/main_outputs/consensus_clusters_summary_k", K_CONSENSUS, ".csv"))
      print(paste0("Consensus clusters summary saved to Clustering/mcmc/Kmeans/main_outputs/consensus_clusters_summary_k", K_CONSENSUS, ".csv"))
    } else {
...
# --- 9.7. Characterize Consensus Clusters for K=6 ---
if (exists("hierarchical_clust") && exists("all_mcmc_samples_df")) {
    print("--- Characterizing Consensus Clusters for K=6 ---")
    K_FOR_ANALYSIS <- 6
    
    consensus_clusters_k6 <- cutree(hierarchical_clust, k = K_FOR_ANALYSIS)
    consensus_assignments_df_k6 <- data.frame(
        Pathogen_Name = names(consensus_clusters_k6),
        Consensus_Cluster = consensus_clusters_k6
    )
    
    write.csv(consensus_assignments_df_k6, paste0("Clustering/mcmc/Kmeans/main_outputs/consensus_cluster_assignments_k", K_FOR_ANALYSIS, ".csv"))
    
    pathogen_mcmc_samples_with_k6_clusters <- all_mcmc_samples_df %>%
        left_join(consensus_assignments_df_k6, by = "Pathogen_Name")
        
    params_to_summarize <- c("R0_sampled", "SI_Clust_sampled", "CFR_sampled",
                           "k_sampled", "IP_sampled", "LP_sampled", "InfP_sampled",
                           "Route_resp", "Route_direct", "Route_sexual", "Route_animal", "Route_vector")

    if(nrow(pathogen_mcmc_samples_with_k6_clusters) > 0 && "Consensus_Cluster" %in% names(pathogen_mcmc_samples_with_k6_clusters)){
        summary_list_k6 <- list()
        for (param in params_to_summarize) {
            if (param %in% names(pathogen_mcmc_samples_with_k6_clusters)) {
                summary_for_param <- pathogen_mcmc_samples_with_k6_clusters %>%
                    ...
                    rename_with(~paste0(param, "_", .), .cols = c(mean_val, lower_ci, upper_ci, median_val, sd_val, n_mcmc_samples_in_calc))
                summary_list_k6[[param]] <- summary_for_param
            }
        }
        
        if(length(summary_list_k6) > 0){
            final_summary_df_k6 <- Reduce(function(x, y) full_join(x, y, by = "Consensus_Cluster"), summary_list_k6)
            print("Summary of Consensus Cluster Characteristics for K=6 (Mean, 95% CI, Median, SD):")
            print(as.data.frame(final_summary_df_k6))
            write_csv(final_summary_df_k6, paste0("Clustering/mcmc/Kmeans/main_outputs/consensus_clusters_summary_k", K_FOR_ANALYSIS, ".csv"))
        }
    }
}
