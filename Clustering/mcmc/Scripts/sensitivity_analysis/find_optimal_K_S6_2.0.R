# Script to find the optimal number of clusters (K) using the silhouette method for S6_2.0 Analysis
...
print("--- Starting Optimal K Analysis (Silhouette Method) for S6_2.0 ---")

# --- 2. Load the Dissimilarity Matrix ---
# This matrix is the output from the consensus clustering step in the S6_2.0 analysis script.
dissimilarity_matrix_file <- "Clustering/mcmc/Kmeans/S6_2_outputs/dissimilarity_matrix.csv"
if (!file.exists(dissimilarity_matrix_file)) {
  stop(paste("Error: Dissimilarity matrix file not found at", dissimilarity_matrix_file, ". Please run the supplementary_figure_S6_2.0.R script first."))
}
...
k_range <- 2:9 # Range of K to test (updated to 9 as we have 10 pathogens)
...
plot_filename <- "Clustering/mcmc/Kmeans/S6_2_outputs/figure_optimal_k.png"
ggsave(plot_filename, plot = optimal_k_plot, width = 8, height = 6)
print(paste("Optimal K plot saved to", plot_filename))

results_filename <- "Clustering/mcmc/Kmeans/S6_2_outputs/optimal_k_silhouette_results.csv"
