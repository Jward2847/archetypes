...
# Filter for specific pathogens of interest 
...
if (nrow(all_mcmc_samples_df) > 0) {
    ...
    # --- 7. Save Results ---
    write_csv(all_mcmc_samples_df, "Clustering/mcmc/Kmeans/S5-8_outputs/mcmc_parameter_samples.csv")
    print("MCMC parameter samples saved to Clustering/mcmc/Kmeans/S5-8_outputs/mcmc_parameter_samples.csv")
} else {
...
if (exists("all_mcmc_samples_df") && nrow(all_mcmc_samples_df) > 0) {
  ...
  perform_kmeans_for_iteration <- function(iter_val, scaled_data, original_data, numerical_cols) {
    ...
    unscaled_centroids_df <- original_data %>%
      ...
      summarise(
        across(all_of(numerical_cols), ~mean(.x, na.rm = TRUE)),
  ...
  clustering_results_list <- future_map(
    mcmc_iterations,
    ~perform_kmeans_for_iteration(.x, scaled_clustering_data_full, clustering_data_full, numerical_param_cols),
  ...
  if (length(successful_results) > 0) {
    ...
    if (length(feature_cols_for_centroids) > 0 && nrow(all_iteration_centroids) > 0) {
        ...
        summary_filename <- paste0("Clustering/mcmc/Kmeans/S5-8_outputs/cluster_centroids_summary_with_ci_k", CHOSEN_K, ".csv")
    ...
    if (nrow(all_iteration_assignments) > 0) {
        ...
        modal_filename <- paste0("Clustering/mcmc/Kmeans/S5-8_outputs/pathogen_modal_cluster_assignments_k", CHOSEN_K, ".csv")
      ...
      if (!requireNamespace("ggplot2", quietly = TRUE)) {
          ...
          ggsave(paste0("Clustering/mcmc/Kmeans/S5-8_outputs/pca_modal_cluster_plot_k",CHOSEN_K,".png"), plot = pca_plot_modal, width=10, height=8)
          print(paste0("PCA plot with modal cluster assignments saved to Clustering/mcmc/Kmeans/S5-8_outputs/pca_modal_cluster_plot_k",CHOSEN_K,".png"))
            ...
            if (nrow(example_iter_features) >= CHOSEN_K) {
                ...
                if(!is.null(kmeans_example_iter_result)){
                    if (!requireNamespace("cluster", quietly = TRUE)) {
                        ...
                        ggsave(paste0("Clustering/mcmc/Kmeans/S5-8_outputs/silhouette_plot_k",CHOSEN_K,"_iter",example_iter_val,".png"), plot = sil_plot_example_iter, width=8, height=6)
...
if (exists("all_iteration_assignments") && nrow(all_iteration_assignments) > 0 && 
  ...
  write.csv(coassignment_matrix, "Clustering/mcmc/Kmeans/S5-8_outputs/coassignment_matrix.csv")
  ...
  write.csv(dissimilarity_matrix, "Clustering/mcmc/Kmeans/S5-8_outputs/dissimilarity_matrix.csv")
  ...
  # Create the heatmap
  coassignment_heatmap_S5 <- ggplot(melted_coassign, aes(x = Var1, y = Var2, fill = value)) +
    geom_tile(color = "white") +
    geom_text(aes(label = round(value, 2), color = value > 0.5), size = 2) +
  ...
  coassignment_heatmap_filename <- "Clustering/mcmc/Kmeans/S5-8_outputs/coassignment_heatmap.png"
  ggsave(coassignment_heatmap_filename, plot = coassignment_heatmap_S5, width = 8, height = 7, bg = "white")
  ...
  # print(optimal_k_plot) # This plot will be combined and saved later
  
  # Save the results data
  results_filename_optimal_k <- "Clustering/mcmc/Kmeans/S5-8_outputs/optimal_k_silhouette_results.csv"
  ...
  combined_plot <- (optimal_k_plot + coassignment_heatmap_S5) / dendro_ggplot + plot_layout(heights = c(1, 1.5))
  
  combined_filename <- "Clustering/mcmc/Kmeans/figures/figure.S5.png"
  ...
  for (k_val in k_values_for_dendro) {
    ...
    dend_colored_k <- set(dend_colored_k, "branches_lwd", 3)
  ...
  if (length(dendro_plot_list) == 3) {
    ...
    dendro_figure_filename <- "Clustering/mcmc/Kmeans/figures/figure.S6.png"
  ...
  write.csv(consensus_assignments_df, paste0("Clustering/mcmc/Kmeans/S5-8_outputs/consensus_cluster_assignments_k", K_CONSENSUS, ".csv"))
  ...
  if(nrow(pathogen_mcmc_samples_with_consensus_clusters) > 0 && "Consensus_Cluster" %in% names(pathogen_mcmc_samples_with_consensus_clusters)){
    ...
    if(length(consensus_cluster_summary_list) > 0){
      ...
      write_csv(final_consensus_summary_df, paste0("Clustering/mcmc/Kmeans/S5-8_outputs/consensus_clusters_summary_k", K_CONSENSUS, ".csv"))
      print(paste0("Consensus clusters summary saved to Clustering/mcmc/Kmeans/S5-8_outputs/consensus_clusters_summary_k", K_CONSENSUS, ".csv"))
    } else {
...
# --- 10. Re-analysis without Presymptomatic Proportion ---
if (exists("all_mcmc_samples_df") && nrow(all_mcmc_samples_df) > 0) {
  
  print("--- Starting Re-analysis without Presymptomatic Proportion ---")
  
  CHOSEN_K <- 4 # Same K as main analysis
  
  # --- 10.1. Prepare Data for Clustering (No Presymptomatic Proportion) ---
  clustering_data_full_no_presym <- all_mcmc_samples_df %>%
    select(
      MCMC_Iteration,
      Pathogen_Name,
      R0_sampled, 
      SI_Clust_sampled, 
      CFR_sampled, 
      # Presymp_Proportion_sampled is excluded
      Route_resp,
      Route_direct,
      Route_sexual,
      Route_animal,
  ...
  numerical_param_cols_no_presym <- c("R0_sampled", "SI_Clust_sampled", "CFR_sampled")
  
  # Impute NAs
  clustering_data_full_no_presym <- clustering_data_full_no_presym %>%
    group_by(Pathogen_Name) %>%
    mutate(across(all_of(numerical_param_cols_no_presym), 
                  ~ifelse(is.na(.), median(., na.rm = TRUE), .))) %>%
    ungroup()

  # Remove rows with remaining NAs
  clustering_data_full_no_presym <- clustering_data_full_no_presym %>%
      filter(if_all(all_of(numerical_param_cols_no_presym), ~!is.na(.)))
      
  # Scale numerical features
  scaled_clustering_data_full_no_presym <- clustering_data_full_no_presym
  scaled_clustering_data_full_no_presym[numerical_param_cols_no_presym] <- scale(scaled_clustering_data_full_no_presym[numerical_param_cols_no_presym])

  # --- 10.2. K-Means Clustering for Each MCMC Iteration (No Presymptomatic Proportion) ---
  mcmc_iterations_no_presym <- unique(scaled_clustering_data_full_no_presym$MCMC_Iteration)
  
  print("Starting parallel per-iteration K-means (no presymptomatic proportion)...")
  clustering_results_list_no_presym <- future_map(
    mcmc_iterations_no_presym,
    ~perform_kmeans_for_iteration(.x, scaled_clustering_data_full_no_presym, clustering_data_full_no_presym, numerical_param_cols_no_presym),
    .options = furrr_options(seed = CLUSTERING_SEED),
    .progress = TRUE
  )
  
  successful_results_no_presym <- purrr::compact(clustering_results_list_no_presym)
  
  if (length(successful_results_no_presym) > 0) {
    all_iteration_assignments_no_presym <- purrr::map_dfr(successful_results_no_presym, "assignments")
    
    # --- 10.3. Ensemble Clustering (No Presymptomatic Proportion) ---
    print("--- Starting Ensemble/Consensus Clustering Analysis (No Presymptomatic Proportion) ---")
    
    pathogen_names_no_presym <- sort(unique(all_iteration_assignments_no_presym$Pathogen_Name))
    num_pathogens_no_presym <- length(pathogen_names_no_presym)
    num_mcmc_iterations_no_presym <- length(unique(all_iteration_assignments_no_presym$MCMC_Iteration))

    # Construct Co-assignment Matrix
    coassignment_matrix_no_presym <- matrix(0, nrow = num_pathogens_no_presym, ncol = num_pathogens_no_presym,
                                     dimnames = list(pathogen_names_no_presym, pathogen_names_no_presym))
    
    assignments_by_iter_no_presym <- split(all_iteration_assignments_no_presym[, c("Pathogen_Name", "Cluster_Assigned")], 
                                    all_iteration_assignments_no_presym$MCMC_Iteration)

    for (iter_assignments in assignments_by_iter_no_presym) {
      for (i in 1:(num_pathogens_no_presym - 1)) {
        for (j in (i + 1):num_pathogens_no_presym) {
          pathogen1_name <- pathogen_names_no_presym[i]
          pathogen2_name <- pathogen_names_no_presym[j]
          
          p1_assignment <- iter_assignments$Cluster_Assigned[iter_assignments$Pathogen_Name == pathogen1_name]
          p2_assignment <- iter_assignments$Cluster_Assigned[iter_assignments$Pathogen_Name == pathogen2_name]
          
          if (length(p1_assignment) > 0 && length(p2_assignment) > 0 && p1_assignment == p2_assignment) {
            coassignment_matrix_no_presym[pathogen1_name, pathogen2_name] <- coassignment_matrix_no_presym[pathogen1_name, pathogen2_name] + 1
            coassignment_matrix_no_presym[pathogen2_name, pathogen1_name] <- coassignment_matrix_no_presym[pathogen1_name, pathogen2_name]
          }
        }
      }
    }
    diag(coassignment_matrix_no_presym) <- num_mcmc_iterations_no_presym
    
    # Calculate Dissimilarity Matrix
    dissimilarity_matrix_no_presym <- 1 - (coassignment_matrix_no_presym / num_mcmc_iterations_no_presym)
    diag(dissimilarity_matrix_no_presym) <- 0

    # Perform Hierarchical Clustering
    pathogen_dist_no_presym <- as.dist(dissimilarity_matrix_no_presym)
    hierarchical_clust_no_presym <- hclust(pathogen_dist_no_presym, method = "average")
    
    # --- 10.4. Generate and Save Figures (No Presymptomatic Proportion) ---
    
    # Optimal K Analysis
    k_range_no_presym <- 2:10
    # The silhouette function can take the original distance matrix directly.
    avg_silhouette_widths_no_presym <- sapply(k_range_no_presym, function(k) {
        cluster_assignments <- cutree(hierarchical_clust_no_presym, k = k)
        silhouette_info <- silhouette(cluster_assignments, dmatrix = dissimilarity_matrix_no_presym)
        if (is.null(silhouette_info) || !is.matrix(silhouette_info) || nrow(silhouette_info) == 0) {
            return(NA)
        }
        summary(silhouette_info)$avg.width
    })
    silhouette_results_df_no_presym <- data.frame(K = k_range_no_presym, Average_Silhouette_Width = avg_silhouette_widths_no_presym)
    
    # --- Find and report the optimal K for the no-presym analysis ---
    if(any(!is.na(silhouette_results_df_no_presym$Average_Silhouette_Width))){
        optimal_k_value_no_presym <- silhouette_results_df_no_presym$K[which.max(silhouette_results_df_no_presym$Average_Silhouette_Width)]
        print(paste("Optimal K (no presym) based on silhouette analysis:", optimal_k_value_no_presym))
    } else {
        optimal_k_value_no_presym <- 4 # Fallback K
        warning("Could not determine optimal K for no-presym analysis from silhouette. Falling back to K=4.")
    }
    
    optimal_k_plot_no_presym <- ggplot(silhouette_results_df_no_presym, aes(x = K, y = Average_Silhouette_Width)) +
      geom_line(color = "steelblue", size = 1) + geom_point(color = "steelblue", size = 3) +
      geom_point(data = silhouette_results_df_no_presym[which.max(silhouette_results_df_no_presym$Average_Silhouette_Width), ], aes(x = K, y = Average_Silhouette_Width), color = "red", size = 5, shape = 8) +
      scale_x_continuous(breaks = k_range_no_presym) +
      labs(title = "", x = "Number of Clusters (K)", y = "Average Silhouette Width", tag = "A") +
      theme_minimal(base_size = 14)

    # Co-assignment Heatmap
    coassignment_prob_matrix_no_presym <- coassignment_matrix_no_presym / num_mcmc_iterations_no_presym
    hc_order_no_presym <- hierarchical_clust_no_presym$order
    ordered_pathogen_names_no_presym <- pathogen_names_no_presym[hc_order_no_presym]
    coassignment_prob_matrix_ordered_no_presym <- coassignment_prob_matrix_no_presym[ordered_pathogen_names_no_presym, ordered_pathogen_names_no_presym]
    
    pathogen_names_full_ordered_no_presym <- pathogen_full_name_map[rownames(coassignment_prob_matrix_ordered_no_presym)]
    pathogen_names_full_ordered_no_presym[is.na(pathogen_names_full_ordered_no_presym)] <- rownames(coassignment_prob_matrix_ordered_no_presym)[is.na(pathogen_names_full_ordered_no_presym)]
    rownames(coassignment_prob_matrix_ordered_no_presym) <- colnames(coassignment_prob_matrix_ordered_no_presym) <- pathogen_names_full_ordered_no_presym
    
    melted_coassign_no_presym <- reshape2::melt(coassignment_prob_matrix_ordered_no_presym)
    melted_coassign_no_presym$Var1 <- factor(melted_coassign_no_presym$Var1, levels = pathogen_names_full_ordered_no_presym)
    melted_coassign_no_presym$Var2 <- factor(melted_coassign_no_presym$Var2, levels = rev(pathogen_names_full_ordered_no_presym))

    coassignment_heatmap_S5_no_presym <- ggplot(melted_coassign_no_presym, aes(x = Var1, y = Var2, fill = value)) +
      geom_tile(color = "white") + geom_text(aes(label = round(value, 2), color = value > 0.5), size = 2) +
      scale_fill_viridis_c(name = "Co-assignment\nProbability", limits = c(0, 1)) +
      scale_color_manual(values = c("white", "black"), guide = "none") +
      labs(title = "", x = NULL, y = NULL, tag = "B") +
      theme_minimal(base_size = 10) +
      theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, size=8), axis.text.y = element_text(size=8), axis.ticks = element_blank(), panel.grid = element_blank())

    # Dendrogram
    K_CONSENSUS_no_presym <- optimal_k_value_no_presym # Use the dynamically found optimal K
    dend_no_presym <- as.dendrogram(hierarchical_clust_no_presym)
    labels(dend_no_presym) <- pathogen_full_name_map[labels(dend_no_presym)]
    
    # Safely get colors for dendrogram
    dend_colors <- RColorBrewer::brewer.pal(n = max(3, K_CONSENSUS_no_presym), name = "Set2")
    if (K_CONSENSUS_no_presym > length(dend_colors)) {
      dend_colors <- rep(dend_colors, length.out = K_CONSENSUS_no_presym)
    }
    
    dend_colored_no_presym <- color_branches(dend_no_presym, k = K_CONSENSUS_no_presym, col = dend_colors)
    dend_colored_no_presym <- set(dend_colored_no_presym, "labels_cex", 0.7) %>% set("branches_lwd", 3)
    
    dendro_grob_no_presym <- as.grob(~{
      par(mar = c(5, 4, 4, 10))
      plot(dend_colored_no_presym, horiz = TRUE, main = "", xlab = "Dissimilarity (1 - Proportion Co-assigned)")
    })
    dendro_ggplot_no_presym <- as.ggplot(dendro_grob_no_presym) + labs(tag = "C")

    # Combine and save Figure S5 (no presym)
    combined_plot_no_presym <- (optimal_k_plot_no_presym + coassignment_heatmap_S5_no_presym) / dendro_ggplot_no_presym + plot_layout(heights = c(1, 1.5))
    combined_filename_no_presym <- "Clustering/mcmc/Kmeans/figures/figure.S7.png"
    ggsave(combined_filename_no_presym, plot = combined_plot_no_presym, width = 12, height = 12, bg = "white")
    print(paste("Combined plot (no presym) saved to", combined_filename_no_presym))

    # Generate and save dendrograms for K=4, 5, 6 (no presym)
    dendro_plot_list_no_presym <- list()
    for (k_val in c(4, 5, 6)) {
      cluster_colors_k <- RColorBrewer::brewer.pal(n = max(3, k_val), name = "Set2")
      if (k_val > length(cluster_colors_k)) {
        cluster_colors_k <- rep(cluster_colors_k, length.out = k_val)
      }
      
      dend_colored_k <- color_branches(dend_no_presym, k = k_val, col = cluster_colors_k[1:k_val])
      dend_colored_k <- set(dend_colored_k, "labels_cex", 0.7)
      dend_colored_k <- set(dend_colored_k, "branches_lwd", 3)
      
      dendro_grob_k <- as.grob(~{
        par(mar = c(5, 4, 4, 10))
        plot(dend_colored_k, horiz = TRUE, main = paste0("K = ", k_val), xlab = "Dissimilarity (1 - Proportion Co-assigned)")
      })
      dendro_plot_list_no_presym[[as.character(k_val)]] <- as.ggplot(dendro_grob_k)
    }
    
    combined_dendro_plot_no_presym <- (dendro_plot_list_no_presym[["4"]] + dendro_plot_list_no_presym[["5"]]) / (dendro_plot_list_no_presym[["6"]] + plot_spacer())
    dendro_figure_filename_no_presym <- "Clustering/mcmc/Kmeans/figures/figure.S8.png"
    ggsave(dendro_figure_filename_no_presym, plot = combined_dendro_plot_no_presym, width = 12, height = 10, bg = "white")
    print(paste("Combined dendrogram plot (no presym) for K=4,5,6 saved to", dendro_figure_filename_no_presym))

  } else {
    print("No successful clustering iterations for the 'no presymptomatic proportion' analysis. Skipping ensemble analysis and figure generation.")
  }
  
  print("--- Finished Re-analysis without Presymptomatic Proportion ---")
}
