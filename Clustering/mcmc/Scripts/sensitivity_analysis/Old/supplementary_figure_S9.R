...
# MCMC K-Means Clustering Analysis V3
...
# --- 3. Load Data ---
params_df <- read_csv("Clustering/mcmc/Kmeans/data/pathogen_params_sens.csv")
...
# Helper function to sample from a distribution specified by family and params
sample_from_dist_info <- function(dist_info, n = 1) {
  if (is.null(dist_info) || is.na(dist_info$family) || length(dist_info$params) == 0) {
    return(rep(NA, n))
  }
  
  params <- dist_info$params
  
  # Helper to create a random generation function from dist_info
  get_r_func <- function(d_info) {
    params <- d_info$params
    switch(d_info$family,
      "Lognormal" = function(n) rlnorm(n, meanlog = params$meanlog, sdlog = params$sdlog),
      "Gamma"     = function(n) rgamma(n, shape = params$shape, rate = if (!is.null(params$rate)) params$rate else 1/params$scale),
      "Normal"    = function(n) rnorm(n, mean = params$mean, sd = params$SD),
      "Weibull"   = function(n) rweibull(n, shape = params$shape, scale = params$scale),
      "runif"     = function(n) runif(n, min = params$min, max = params$max),
      NULL
    )
  }
  
  r_func <- get_r_func(dist_info)
  
  if (is.null(r_func)) {
    warning(paste("Could not get random generation function for", dist_info$family))
    return(rep(NA, n))
  }
  ...
  if(dist_info$family == "Normal" && any(samples < 0, na.rm = TRUE)) {
      samples[samples < 0] <- 0
  }
...
sample_clust_parameter <- function(pathogen_row, param_prefix) {
  ...
  } else if (uncert_type %in% c("95CI", "90CI", "IQR", "Range", "range")) {
...
for (iter in 1:N_MCMC_ITERATIONS) {
  ...
  for (i in 1:nrow(params_df)) {
    ...
    incubation_period_sampled <- NA
    # Try to sample from full distribution first
    if (!is.na(ip_full_dist_info$family) && length(ip_full_dist_info$params) > 0) {
      incubation_period_sampled <- sample_from_dist_info(ip_full_dist_info, n=1)
    }
    
    # If sampling from full dist failed or wasn't possible, try sampling from cluster parameters
    if (is.na(incubation_period_sampled)) {
      incubation_period_sampled <- sample_clust_parameter(pathogen_row, "IP")
    }
    
    presymp_prop_sampled <- NA # SI Full dist not available in this dataset
    ...
    pathogen_params_df <- data.frame(
      ...
      IncubationPeriod_sampled = ifelse(is.null(incubation_period_sampled) || is.na(incubation_period_sampled) || !is.finite(incubation_period_sampled), NA_real_, incubation_period_sampled),
      ...
      Route_food_water = as.integer(pathogen_row$Route_food_water)
    )
...
if (length(mcmc_results) > 0 && !all(sapply(mcmc_results, is.null))){
    ...
    if(nrow(all_mcmc_samples_df) > 0){
        ...
        # --- 7. Save Results ---
        write_csv(all_mcmc_samples_df, "Clustering/mcmc/Kmeans/S9_outputs/mcmc_parameter_samples_sens.csv")
        print("MCMC parameter samples saved to Clustering/mcmc/Kmeans/S9_outputs/mcmc_parameter_samples_sens.csv")
    } else {
...
# --- 8. Determine Optimal K ---
if (exists("all_mcmc_samples_df") && nrow(all_mcmc_samples_df) > 0) {
  print("--- Determining Optimal Number of Clusters (K) using Silhouette Method ---")
  ...
  # Summarize the MCMC samples to get a single average dataset for finding K
  pathogen_summary_for_k <- all_mcmc_samples_df %>%
    group_by(Pathogen_Name) %>%
    summarise(
      R0_mean = mean(R0_sampled, na.rm = TRUE),
      CFR_mean = mean(CFR_sampled, na.rm = TRUE),
      IncubationPeriod_mean = mean(IncubationPeriod_sampled, na.rm = TRUE),
      ...
      Route_food_water_mean = mean(Route_food_water, na.rm=TRUE),
      .groups = 'drop'
    )
  
  features_for_k_finding <- pathogen_summary_for_k %>% select(-Pathogen_Name)
  
  # Impute NAs with median just in case (e.g. if a pathogen had all NA samples for a param)
  for(col in names(features_for_k_finding)){
    if(any(is.na(features_for_k_finding[[col]]))){
      features_for_k_finding[[col]][is.na(features_for_k_finding[[col]])] <- median(features_for_k_finding[[col]], na.rm = TRUE)
    }
  }

  scaled_features_for_k_finding <- scale(features_for_k_finding)
  
  max_k <- nrow(scaled_features_for_k_finding) - 1
  k_values <- 2:min(15, max_k)
  avg_sil_widths <- numeric(length(k_values))
  
  for (i in 1:length(k_values)) {
    k <- k_values[i]
    set.seed(123)
    km_res <- kmeans(scaled_features_for_k_finding, centers = k, nstart = 25)
    sil <- silhouette(km_res$cluster, dist(scaled_features_for_k_finding))
    avg_sil_widths[i] <- mean(sil[, 3])
  }
  
  silhouette_results <- data.frame(k = k_values, avg_sil_width = avg_sil_widths)
  
  optimal_k_silhouette <- silhouette_results$k[which.max(silhouette_results$avg_sil_width)]
  
  print(paste("Optimal K based on silhouette score:", optimal_k_silhouette))
  write_csv(silhouette_results, "Clustering/mcmc/Kmeans/S9_outputs/optimal_k_silhouette_results_sens.csv")
  
  # Plot the results
  sil_plot <- ggplot(silhouette_results, aes(x = k, y = avg_sil_width)) +
    geom_line() +
    geom_point() +
    geom_vline(xintercept = optimal_k_silhouette, linetype = "dashed", color = "red") +
    labs(title = "Optimal K using Average Silhouette Width",
         x = "Number of Clusters (K)",
         y = "Average Silhouette Width") +
    theme_minimal()
    
  print(sil_plot)
  ggsave("Clustering/mcmc/Kmeans/figures/figure.S8.png", plot = sil_plot, width = 8, height = 6)
  
  CHOSEN_K <- optimal_k_silhouette
  
} else {
  print("Skipping optimal K determination as MCMC samples are not available.")
  CHOSEN_K <- 6 # Fallback to 6 if MCMC samples are missing
}
...
if (exists("all_mcmc_samples_df") && nrow(all_mcmc_samples_df) > 0) {
  ...
  numerical_param_cols <- c("R0_sampled", "CFR_sampled", "IncubationPeriod_sampled")
      ...
      if (!is.null(kmeans_iter_result)) {
          ...
          iter_unscaled_centroids <- original_data_this_iter_for_centroids %>%
            ...
            summarise(
              ...
              Route_food_water = mean(Route_food_water, na.rm = TRUE),
              .groups = 'drop'
            ) %>%
  ...
  if (length(all_iteration_centroids) > 0 && length(all_iteration_assignments) > 0) {
      ...
      if(length(feature_cols_for_centroids) > 0 && nrow(combined_centroids_df) > 0) {
          ...
          write_csv(centroid_summary, "Clustering/mcmc/Kmeans/S9_outputs/cluster_centroids_summary_with_ci_k6_sens.csv")
          print("Cluster centroids summary saved to Clustering/mcmc/Kmeans/S9_outputs/cluster_centroids_summary_with_ci_k6_sens.csv")
      } else {
      ...
      if (nrow(combined_assignments_df) > 0) {
          ...
          write_csv(modal_assignments, "Clustering/mcmc/Kmeans/S9_outputs/pathogen_modal_cluster_assignments_k6_sens.csv")
      } else {
      ...
      plot_features_numerical <- c("R0_mean_overall", "CFR_mean_overall", "IncubationPeriod_mean_overall")
      plot_features_routes <- c("Route_resp_mean", "Route_direct_mean", "Route_sexual_mean", "Route_animal_mean", "Route_vector_mean", "Route_food_water_mean")
      ...
      if (!requireNamespace("ggplot2", quietly = TRUE)) {
          ...
          ggsave(paste0("Clustering/mcmc/Kmeans/S9_outputs/pca_modal_cluster_plot_k",CHOSEN_K,"_sens.png"), plot = pca_plot_modal, width=10, height=8)
          print(paste0("PCA plot with modal cluster assignments saved to Clustering/mcmc/Kmeans/S9_outputs/pca_modal_cluster_plot_k",CHOSEN_K,"_sens.png"))
            ...
            if (nrow(example_iter_features) >= CHOSEN_K) {
                ...
                if(!is.null(kmeans_example_iter_result)){
                    if (!requireNamespace("cluster", quietly = TRUE)) {
                        ...
                        ggsave(paste0("Clustering/mcmc/Kmeans/S9_outputs/silhouette_plot_k",CHOSEN_K,"_iter",example_iter_val,"_sens.png"), plot = sil_plot_example_iter, width=8, height=6)
...
if (exists("combined_assignments_df") && nrow(combined_assignments_df) > 0 && 
  ...
  write.csv(coassignment_matrix, "Clustering/mcmc/Kmeans/S9_outputs/coassignment_matrix_sens.csv")
  ...
  write.csv(dissimilarity_matrix, "Clustering/mcmc/Kmeans/S9_outputs/dissimilarity_matrix_sens.csv")
  ...
  png_filename <- paste0("Clustering/mcmc/Kmeans/figures/figure.S9", ".png")
  ...
  write.csv(consensus_assignments_df, paste0("Clustering/mcmc/Kmeans/S9_outputs/consensus_cluster_assignments_k", K_CONSENSUS, "_sens.csv"))
  ...
  params_to_summarize <- c("R0_sampled", "CFR_sampled", 
                           "IncubationPeriod_sampled",
                           "Route_resp", "Route_direct", "Route_sexual", "Route_animal", "Route_vector", "Route_food_water")
  ...
  if(nrow(pathogen_mcmc_samples_with_consensus_clusters) > 0 && "Consensus_Cluster" %in% names(pathogen_mcmc_samples_with_consensus_clusters)){
    ...
    if(length(consensus_cluster_summary_list) > 0){
      ...
      write_csv(final_consensus_summary_df, paste0("Clustering/mcmc/Kmeans/S9_outputs/consensus_clusters_summary_k", K_CONSENSUS, "_sens.csv"))
      print(paste0("Consensus clusters summary saved to Clustering/mcmc/Kmeans/S9_outputs/consensus_clusters_summary_k", K_CONSENSUS, "_sens.csv"))
    } else {
