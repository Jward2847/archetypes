...
if (nrow(all_mcmc_samples_df) > 0) {
    ...
    # --- 7. Save Results ---
    write_csv(all_mcmc_samples_df, "Clustering/mcmc/Kmeans/S7_outputs/mcmc_parameter_samples.csv")
    print("MCMC parameter samples saved to Clustering/mcmc/Kmeans/S7_outputs/mcmc_parameter_samples.csv")
} else {
...
if (exists("all_mcmc_samples_df") && nrow(all_mcmc_samples_df) > 0) {
  ...
  if (length(successful_results) > 0) {
    ...
    if (length(feature_cols_for_centroids) > 0 && nrow(all_iteration_centroids) > 0) {
        ...
        summary_filename <- paste0("Clustering/mcmc/Kmeans/S7_outputs/cluster_centroids_summary_with_ci_k", CHOSEN_K, ".csv")
    ...
    if (nrow(all_iteration_assignments) > 0) {
        ...
        modal_filename <- paste0("Clustering/mcmc/Kmeans/S7_outputs/pathogen_modal_cluster_assignments_k", CHOSEN_K, ".csv")
      ...
      if (!requireNamespace("ggplot2", quietly = TRUE)) {
          ...
          ggsave(paste0("Clustering/mcmc/Kmeans/S7_outputs/pca_modal_cluster_plot_k",CHOSEN_K,".png"), plot = pca_plot_modal, width=10, height=8)
          print(paste0("PCA plot with modal cluster assignments saved to Clustering/mcmc/Kmeans/S7_outputs/pca_modal_cluster_plot_k",CHOSEN_K,".png"))
            ...
            if (nrow(example_iter_features) >= CHOSEN_K) {
                ...
                if(!is.null(kmeans_example_iter_result)){
                    if (!requireNamespace("cluster", quietly = TRUE)) {
                        ...
                        ggsave(paste0("Clustering/mcmc/Kmeans/S7_outputs/silhouette_plot_k",CHOSEN_K,"_iter",example_iter_val,".png"), plot = sil_plot_example_iter, width=8, height=6)
...
if (exists("all_iteration_assignments") && nrow(all_iteration_assignments) > 0 && 
  ...
  write.csv(coassignment_matrix, "Clustering/mcmc/Kmeans/S7_outputs/coassignment_matrix.csv")
  ...
  write.csv(dissimilarity_matrix, "Clustering/mcmc/Kmeans/S7_outputs/dissimilarity_matrix.csv")
  ...
  write.csv(consensus_assignments_df, paste0("Clustering/mcmc/Kmeans/S7_outputs/consensus_cluster_assignments_k", K_CONSENSUS, ".csv"))
  ...
  if(nrow(pathogen_mcmc_samples_with_consensus_clusters) > 0 && "Consensus_Cluster" %in% names(pathogen_mcmc_samples_with_consensus_clusters)){
    ...
    if(length(consensus_cluster_summary_list) > 0){
      ...
      write_csv(final_consensus_summary_df, paste0("Clustering/mcmc/Kmeans/S7_outputs/consensus_clusters_summary_k", K_CONSENSUS, ".csv"))
      print(paste0("Consensus clusters summary saved to Clustering/mcmc/Kmeans/S7_outputs/consensus_clusters_summary_k", K_CONSENSUS, ".csv"))
    } else {
