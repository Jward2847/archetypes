# MCMC K-Means Clustering Analysis V2
...
if (exists("all_mcmc_samples_df") && nrow(all_mcmc_samples_df) > 0) {
  ...
  if (length(all_iteration_centroids) > 0 && length(all_iteration_assignments) > 0) {
      ...
      if(length(feature_cols_for_centroids) > 0 && nrow(combined_centroids_df) > 0) {
          ...
          write_csv(centroid_summary, "Clustering/mcmc/Kmeans/S6_outputs/cluster_centroids_summary_with_ci_k6.csv")
          print("Cluster centroids summary saved to Clustering/mcmc/Kmeans/S6_outputs/cluster_centroids_summary_with_ci_k6.csv")
      } else {
      ...
      if (nrow(combined_assignments_df) > 0) {
          ...
          write_csv(modal_assignments, "Clustering/mcmc/Kmeans/S6_outputs/pathogen_modal_cluster_assignments_k6.csv")
      } else {
      ...
      if (!requireNamespace("ggplot2", quietly = TRUE)) {
          ...
          ggsave(paste0("Clustering/mcmc/Kmeans/S6_outputs/pca_modal_cluster_plot_k",CHOSEN_K,".png"), plot = pca_plot_modal, width=10, height=8)
          print(paste0("PCA plot with modal cluster assignments saved to Clustering/mcmc/Kmeans/S6_outputs/pca_modal_cluster_plot_k",CHOSEN_K,".png"))
            ...
            if (nrow(example_iter_features) >= CHOSEN_K) {
                ...
                if(!is.null(kmeans_example_iter_result)){
                    if (!requireNamespace("cluster", quietly = TRUE)) {
                        ...
                        ggsave(paste0("Clustering/mcmc/Kmeans/S6_outputs/silhouette_plot_k",CHOSEN_K,"_iter",example_iter_val,".png"), plot = sil_plot_example_iter, width=8, height=6)
...
if (exists("combined_assignments_df") && nrow(combined_assignments_df) > 0 && 
  ...
  write.csv(coassignment_matrix, "Clustering/mcmc/Kmeans/S6_outputs/coassignment_matrix.csv")
  ...
  write.csv(dissimilarity_matrix, "Clustering/mcmc/Kmeans/S6_outputs/dissimilarity_matrix.csv")
