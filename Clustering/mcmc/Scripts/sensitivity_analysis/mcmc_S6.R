...
if (exists("all_mcmc_samples_df") && nrow(all_mcmc_samples_df) > 0) {
  ...
  if (length(successful_results) > 0) {
    ...
    if (length(feature_cols_for_centroids) > 0 && nrow(all_iteration_centroids) > 0) {
        ...
        summary_filename <- paste0("Clustering/mcmc/Kmeans/S6_outputs/cluster_centroids_summary_with_ci_k", CHOSEN_K, ".csv")
    ...
    if (nrow(all_iteration_assignments) > 0) {
        ...
        modal_filename <- paste0("Clustering/mcmc/Kmeans/S6_outputs/pathogen_modal_cluster_assignments_k", CHOSEN_K, ".csv")
