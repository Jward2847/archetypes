...
if (length(mcmc_results) > 0 && !all(sapply(mcmc_results, is.null))){
    ...
    if(nrow(all_mcmc_samples_df) > 0){
        ...
        # --- 7. Save Results ---
        write_csv(all_mcmc_samples_df, "Clustering/mcmc/Kmeans/mcmc_parameter_samples_v3.csv")
        print("MCMC parameter samples saved to Clustering/mcmc/Kmeans/mcmc_parameter_samples_v3.csv")
    } else {
...
if (exists("all_mcmc_samples_df") && nrow(all_mcmc_samples_df) > 0) {
  ...
  if (length(all_iteration_centroids) > 0 && length(all_iteration_assignments) > 0) {
      ...
      if(length(feature_cols_for_centroids) > 0 && nrow(combined_centroids_df) > 0) {
          ...
          write_csv(centroid_summary, "Clustering/mcmc/Kmeans/cluster_centroids_summary_with_ci_k6_v3.csv")
          print("Cluster centroids summary saved to Clustering/mcmc/Kmeans/cluster_centroids_summary_with_ci_k6_v3.csv")
      } else {
      ...
      if (nrow(combined_assignments_df) > 0) {
          ...
          write_csv(modal_assignments, "Clustering/mcmc/Kmeans/pathogen_modal_cluster_assignments_k6_v3.csv")
      } else {
      ...
      if (!requireNamespace("ggplot2", quietly = TRUE)) {
          ...
          ggsave(paste0("Clustering/mcmc/Kmeans/pca_modal_cluster_plot_k",CHOSEN_K,"_v3.png"), plot = pca_plot_modal, width=10, height=8)
          print(paste0("PCA plot with modal cluster assignments saved to Clustering/mcmc/Kmeans/pca_modal_cluster_plot_k",CHOSEN_K,"_v3.png"))
            ...
            if (nrow(example_iter_features) >= CHOSEN_K) {
                ...
                if(!is.null(kmeans_example_iter_result)){
                    if (!requireNamespace("cluster", quietly = TRUE)) {
                        ...
                        ggsave(paste0("Clustering/mcmc/Kmeans/silhouette_plot_k",CHOSEN_K,"_iter",example_iter_val,"_v3.png"), plot = sil_plot_example_iter, width=8, height=6)
...
if (exists("combined_assignments_df") && nrow(combined_assignments_df) > 0 && 
  ...
  write.csv(coassignment_matrix, "Clustering/mcmc/Kmeans/coassignment_matrix_v3.csv")
  ...
  write.csv(dissimilarity_matrix, "Clustering/mcmc/Kmeans/dissimilarity_matrix_v3.csv")
  ...
  png_filename <- paste0("Clustering/mcmc/Kmeans/figure.1_v3", ".png")
  ...
  write.csv(consensus_assignments_df, paste0("Clustering/mcmc/Kmeans/consensus_cluster_assignments_k", K_CONSENSUS, "_v3.csv"))
  ...
  if(nrow(pathogen_mcmc_samples_with_consensus_clusters) > 0 && "Consensus_Cluster" %in% names(pathogen_mcmc_samples_with_consensus_clusters)){
    ...
    if(length(consensus_cluster_summary_list) > 0){
      ...
      write_csv(final_consensus_summary_df, paste0("Clustering/mcmc/Kmeans/consensus_clusters_summary_k", K_CONSENSUS, "_v3.csv"))
      print(paste0("Consensus clusters summary saved to Clustering/mcmc/Kmeans/consensus_clusters_summary_k", K_CONSENSUS, "_v3.csv"))
    } else {
